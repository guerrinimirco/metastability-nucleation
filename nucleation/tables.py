...
def compute_quantum_nucleation_observables(
        hadronic_table, Qstar_table, sigma=30.0, electric_charge_mode='gcn',
        N_c=1e48, rho_H_func=None, verbose=False):
    """Quantum tunnelling nucleation time over the hadronic grid (WKB)."""
    ...
    elif electric_charge_mode in _COULOMB_MODES:
    ...
    while not it.finished:
        ...
        try:
            ...
            tau_qt_out[idx] = res.tau_qt
            A_out[idx] = res.A
            E_0_out[idx] = res.E_0
            nu_0_out[idx] = res.nu_0
            conv_out[idx] = True
        except Exception:
            pass
        done += 1
    ...
    if verbose:
        print(f"  Quantum nucleation complete: {int(np.sum(conv_out))}/{total} converged")
    return QuantumNucleationObservables(
        eq_type=hadronic_table.eq_type, hadronic_grids=Qstar_table.hadronic_grids,
        sigma=sigma, N_c=N_c, tau_qt=tau_qt_out, A=A_out, E_0=E_0_out,
        nu_0=nu_0_out, converged=conv_out)
