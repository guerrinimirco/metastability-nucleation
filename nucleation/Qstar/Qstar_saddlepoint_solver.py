"""
Q* Computation: Saddlepoint Nucleation using AlphaBag EOS
===================================================================

Given initial hadronic conditions, find the quark droplet Q* thermodynamics
by minimizing the work of formation w.r.t. n_B_Qs, Y_C_Qs, and Y_S_Qs.
...
@dataclass
class QstarSaddlepointTableData:
    """Q* saddlepoint nucleation table.
    ...
    def __repr__(self):
        ...
        return f"QstarSaddlepointTableData(eq_type='{self.eq_type}', grids=({', '.join(shapes)}))"
...
def build_Qstar_interpolators(table, method='linear'):
    """
    Build interpolation functions from a QstarSaddlepointTableData.
...
def solve_Qstar(H, params, electric_charge_neutrality,
    ...
    equations:
    minimize wrt Y_C_Qs: µ_C_Qs(μ_u, μ_d, μ_s) + µ_e_Qs = µ_C_H + µ_e_H
    minimize wrt Y_S_Qs: µ_S_Qs(μ_u, μ_d, μ_s) = µ_S_H
    ...
    def equations(x):
        ...
        C_saddlepoint_equation = Qs.mu_C + mu_e - (H.mu_C + H.mu_e)
        S_saddlepoint_equation = Qs.mu_S - H.mu_S
        ...
        return [
            C_saddlepoint_equation,
            S_saddlepoint_equation,
...
def compute_Qstar_table_betaeq(hadronic_table, params,
    ...
    return QstarSaddlepointTableData(eq_type='betaeq', hadronic_grids=hadronic_grids, data=data)
...
def compute_Qstar_table_trapped(hadronic_table, params,
    ...
    return QstarSaddlepointTableData(eq_type='trapped', hadronic_grids=hadronic_grids, data=data)
...
def compute_Qstar_table_fixedYC(hadronic_table, params,
    ...
    return QstarSaddlepointTableData(eq_type='fixedYC', hadronic_grids=hadronic_grids, data=data)
...
def _export_table(result, params, electric_charge_neutrality, output_file):
    ...
    meta = (
        f"# Q* saddlepoint nucleation table\n"
